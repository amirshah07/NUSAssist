...
const TimetableComponent = ({ 
  ...
  const getUniqueLessonBlocks = (modules: SelectedModule, includeAlternatives = false) => {
    ...
    Object.entries(modules).forEach(([moduleCode, moduleData]) => {
      ...
      const moduleBlocks = timetableEntries.map((entry: any) => {
        ...
        console.log('Created block:', block); // Debug log
        return block;
      });
  ...
  const handleAlternativeSelection = (selectedEvent: CalendarEvent) => {
    ...
    const selectedAlternative = showingAlternatives.alternatives.find(alt => {
      ...
      console.log(`Comparing: ${alt.day} ${alt.startTime} vs ${eventDay} ${eventStartTime}`);
    ...
    if (!selectedAlternative) {
      ...
      console.log('Available alternatives:', showingAlternatives.alternatives);
      console.log('Selected event:', {
        day: moment(selectedEvent.start).format('dddd'),
    ...
    <div style={{ height: '100vh', padding: 16 }}>
      ...
      <div className="timetable-controls">
        <div className="semester-info">
          <span>
            {isOptimized ? 'Optimized timetable' : 'Manual timetable'} for {selectedSemester}
          </span>
        </div>
